...
export function WindowControls() {
  return (
    <Show when={isTauri}>
      <div class="pr-wincontrols">
        <button class="pr-winbtn" aria-label="Minimize" onClick={() => win()?.minimize()}>
          <svg width="10" height="10" viewBox="0 0 10 10"><rect x="1" y="4.5" width="8" height="1" fill="currentColor" /></svg>
        </button>
        <button class="pr-winbtn" aria-label="Maximize" onClick={() => win()?.toggleMaximize()}>
          <svg width="10" height="10" viewBox="0 0 10 10"><rect x="1.5" y="1.5" width="7" height="7" fill="none" stroke="currentColor" /></svg>
        </button>
        <button class="pr-winbtn pr-winbtn-close" aria-label="Close" onClick={() => win()?.close()}>
