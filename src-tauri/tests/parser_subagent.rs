...
fn subagent_run_parses_expected_event_kinds() {
    ...
    let spawns = events.iter().filter(|e| matches!(e, ClaudeEvent::SubagentSpawn { .. })).count();
    assert_eq!(spawns, 1, "exactly one subagent spawn");

    let file_calls = events.iter().filter(|e| matches!(e, ClaudeEvent::ToolCall { file_path: Some(_), .. })).count();
    assert!(file_calls >= 2, "at least two file tool calls (read + edit)");
