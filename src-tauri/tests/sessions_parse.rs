use praetorium_lib::sessions::line_to_turn;
...
fn skips_noise_and_extracts_turns() {
    ...
    assert!(!turns.is_empty(), "expected at least one conversational turn");
    assert!(turns.iter().all(|t| t.role == "user" || t.role == "assistant"));
    assert!(turns.iter().all(|t| !t.text.is_empty()));
    assert!(turns.iter().all(|t| !t.text.contains("local-command-caveat")));
}
