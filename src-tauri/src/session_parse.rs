...
#[serde(rename_all = "camelCase", rename_all_fields = "camelCase", tag = "kind", content = "data")]
...
pub fn parse_transcript_line(line: &str) -> Vec<SessionEvent> {
    let v: Value = match serde_json::from_str(line.trim()) { Ok(v) => v, Err(_) => return vec![] };
    let ty = match v.get("type").and_then(|t| t.as_str()) { Some(t) => t, None => return vec![] };
    if v.get("isMeta").and_then(|b| b.as_bool()).unwrap_or(false) { return vec![]; }
    if v.get("isApiErrorMessage").and_then(|b| b.as_bool()).unwrap_or(false) { return vec![]; }
    let msg = match v.get("message") { Some(m) => m, None => return vec![] };
    if msg.get("model").and_then(|m| m.as_str()) == Some("<synthetic>") { return vec![]; }
    let content = match msg.get("content") { Some(c) => c, None => return vec![] };
    let mut out = vec![];

    let role = msg.get("role").and_then(|r| r.as_str()).unwrap_or(ty).to_string();
            ...
            match b.get("type").and_then(|t| t.as_str()) {
                Some("tool_use") => {
                    ...
                    if name == "Agent" || name == "Task" {
                        let st = input.and_then(|i| i.get("subagent_type")).and_then(|s| s.as_str()).unwrap_or("").to_string();
                        out.push(SessionEvent::SubagentSpawn { tool_use_id: id, subagent_type: st });
                    } else {
                        let fp = input.and_then(|i| i.get("file_path")).and_then(|s| s.as_str()).map(|s| s.to_string());
                        out.push(SessionEvent::ToolActivity { tool_use_id: id, name, file_path: fp });
                    }
                }
                Some("tool_result") => {
...
pub fn tail_new(content: &str, offset: usize) -> (Vec<String>, usize) {
    if offset >= content.len() { return (vec![], content.len()); }
...
mod tests {
    ...
    fn parses_spawn_and_file_activity() {
        ...
        assert!(evs.contains(&SessionEvent::SubagentSpawn { tool_use_id: "a1".into(), subagent_type: "genetor".into() }));
        assert!(evs.contains(&SessionEvent::ToolActivity { tool_use_id: "r1".into(), name: "Read".into(), file_path: Some("/repo/a/b.md".into()) }));
    }
